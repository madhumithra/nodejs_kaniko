        ...
        stage ('Build: Docker') {
            steps {
                container('kaniko') {
                    ...
                    sh 'pwd && ls -l && df -h && cat /kaniko/.docker/config.json && /kaniko/executor -f `pwd`/Dockerfile --context `pwd` --insecure --skip-tls-verify --cache=true --destination=madhupixiee/nodejs_helloworld:latest'
                }
            }
        }
